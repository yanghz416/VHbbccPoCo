...
def plot_shapes_comparison(
    ...
    for sample, subsample, cat, variation, label in shapes:
        if "DATA" in sample: return
        print(sample, subsample, cat, variation)
        print("\t H = \n", H[sample][subsample])
        hep.histplot(H[sample][subsample][cat, variation, :], label=label, ax=axu)
    ...
    # Ratios
    sample, subsample, cat, variation, label = shapes[0]
    nom = H[sample][subsample][cat, variation, :]
    ...
    for sample, subsample, cat, variation, label in shapes:
        ...
        h = H[sample][subsample][cat, variation, :]
        h_val = h.values()
        h_sig2 = h.variances()
