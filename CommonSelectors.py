...
def TwoLepTwoJets(events, params, **kwargs):
    ...
    mask = ( 
             ...
             & (events.elel.pt > params["pt_dilep"])
             & (events.elel.l1pt > 20)
             & (events.elel.charge == 0)
             & (events.elel.mass > params["mll"]["low"])
             & (events.elel.mass < params["mll"]["high"])

            
            )
    return ak.where(ak.is_none(mask), False, mask)
def TwoLepTwoJets_true(events, params, **kwargs):
    if params["lep_flav"] not in ['mu','el','both']:
        print("This lepton flavor is not supported:", params["lep_flav"])
        raise Exception("The lepton flavor is not supported")
    ...
    mask = ( (events.nBJetGood >= 2)
             ...
             & ((events.dijet_bsort.mass < 90)| (events.dijet_bsort.mass > 150))
             #& (events.dijet_bsort.mass < params["mbb"]["high"])
...
def ZLLHBB_2J_addJ(lep_flav='el'):
    return Cut(
    name = 'ZLLHBB_2J_addJ',
    function=TwoLepTwoJets_true,
    params={"lep_flav": lep_flav,
            "pt_dilep": 0,
            "mll": {'low': 75, 'high':105}
            }
...
def dibjet_mass(mass_min=0, mass_max=2000, invert=False):
    return Cut(
        name=f"dibjet_mass_cut",
